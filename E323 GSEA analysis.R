...
plot.gene.enrichment.up <- function(data.to.plot, title) {
  plot <- ggplot(data = data.to.plot, aes(x = Term, y = -log10(Adjusted.P.value))) +
    geom_bar(stat = 'identity', fill = 'red4') +
    geom_text(aes(label=Overlap), hjust=-0.3, vjust = 1, color="Blue", size=5) +
    geom_text(aes(label=paste("E",round(log10(Adjusted.P.value)))), hjust=-0.5, vjust = -0.9, color="black", size=5) +
    geom_text(aes(label=Row1, y = 30, hjust=1), vjust = -0.5, color = "black", fontface = "bold", size = 5) +
    geom_text(aes(label=Row2, y = 30, hjust=1), vjust = 1, color = "Black", fontface = "bold", size = 5) +
    scale_x_discrete(limits=rev(data.to.plot$Term)) +
    ylim(0,30) +
    coord_flip() + 
    labs(title = title,
         y = 'P-value, Adjusted') +
...
plot.gene.enrichment.down <- function(data.to.plot, title) {
  plot <- ggplot(data = data.to.plot, aes(x = Term, y = -log10(Adjusted.P.value))) +
    geom_bar(stat = 'identity', fill = 'blue4') +
    geom_text(aes(label=Overlap), hjust=1.3, vjust = 0.9, color="Blue", size=5) +
    geom_text(aes(label=paste("E",round(log10(Adjusted.P.value)))), hjust=1.5, vjust = -1, color="black", size=5) +
    geom_text(aes(label=Row1, y = 10, hjust=0), vjust = -0.5, color = "black", fontface = "bold", size = 5) +
    geom_text(aes(label=Row2, y = 10, hjust=0), vjust = 1.5, color = "black", fontface = "bold", size = 5) +
    scale_x_discrete(limits=rev(data.to.plot$Term)) +
    ylim(10,0) +
    coord_flip() + 
    labs(title = title,
...
####################################################################################################################################################
### Input
setwd('C:/Users/grossar/Box/Sareen Lab Shared/Data/Vicky/E283-COVID Infection/Round 2 RNAseq/Analysis/Gene Enrichment Analysis/GO tables/')
...
go.vi.up.d3.down <- read.csv('Virus_Perturbations_from_GEO_up_table - d3 down alt.txt', sep = '\t', header = TRUE)
go.mf.d3.up <-    read.table('GO_Molecular_Function_2018_table Up.txt', sep = '\t', header = TRUE)
go.mf.d3.down <-  read.table('GO_Molecular_Function_2018_table Down.txt', sep = '\t', header = TRUE)
go.cc.d3.up <-    read.table('GO_Cellular_Component_2018_table Up.txt', sep = '\t', header = TRUE)
go.cc.d3.down <-  read.table('GO_Cellular_Component_D3 Down.txt', sep = '\t', header = TRUE)
...
(go.vi.up.d3.down <- go.vi.up.d3.down[order(go.vi.up.d3.down$Adjusted.P.value),][1:7,])
(go.mf.d3.up <- go.mf.d3.up[order(go.mf.d3.up$Adjusted.P.value),][1:7,])
(go.mf.d3.down<-go.mf.d3.down[order(go.mf.d3.down$Adjusted.P.value),][1:7,])
(go.cc.d3.up <- go.cc.d3.up[order(go.cc.d3.up$Adjusted.P.value),][1:7,])
(go.cc.d3.down<-go.cc.d3.down[order(go.cc.d3.down$Adjusted.P.value),][1:7,])
...
break.across.rows(go.d3.up,split.positions = c(51,46,100,100,43,129,130))$Row1
go.d3.up <- break.across.rows(go.d3.up,split.positions = c(51,46,100,100,43,129,130))  
go.d3.up <- go.d3.up[1:7,]
ggplot(data = go.d3.up, aes(x = Term, y = -log10(Adjusted.P.value))) +
  geom_bar(stat = 'identity', fill = 'red4') +
  geom_text(aes(label=Overlap), hjust=-0.3, vjust = 1, color="Blue", fontface = "bold", size = font.size) +
  geom_text(aes(label=paste("E",round(log10(Adjusted.P.value)))), hjust=-0.5, vjust = -0.5, color="black", size = font.size, fontface = "bold") +
  geom_text(aes(label=Row1, y = 0.5, hjust=0), vjust = -0.4, color = "White", fontface = "bold", size = font.size) +
  geom_text(aes(label=Row2, y = 0.5, hjust=0), vjust = 1, color = "white", fontface = "bold", size = font.size) +
...
### GO - BP - D3 Down
break.across.rows(go.d3.down,split.positions = c(151,146,100,100,143,129,130,100,100,100))$Row1
go.d3.down <- break.across.rows(go.d3.down,split.positions = c(151,146,100,100,143,129,130,100,100,100))  
go.d3.down <- go.d3.down[1:7,]
ggplot(data = go.d3.down, aes(x = Term, y = -log10(Adjusted.P.value))) +
  geom_bar(stat = 'identity', fill = 'blue4') +
  geom_text(aes(label=Overlap), hjust=1.3, vjust = 1.5, color="Blue", size = font.size) +
  geom_text(aes(label=paste("E",round(log10(Adjusted.P.value)))), hjust=1.5, vjust = -1.2, color="black", size = font.size) +
  geom_text(aes(label=Row1, y = 24, hjust=0), vjust = -0.5, color = "black", fontface = "bold", size = font.size) +
  geom_text(aes(label=Row2, y = 24, hjust=0), vjust = 1.5, color = "black", fontface = "bold", size = font.size) +
...
## COV, Up
break.across.rows(go.cov.d3.up, c(100,42,36,27,24,29,27))$Row1
go.cov.d3.up <- break.across.rows(go.cov.d3.up, c(100,42,36,27,24,29,27))

ggplot(data = go.cov.d3.up, aes(x = Term, y = -log10(Adjusted.P.value))) +
  geom_bar(stat = 'identity', fill = 'red4') +
  geom_text(aes(label=Overlap), hjust=-0.3, vjust = 1, color="Blue", size=5) +
  geom_text(aes(label=paste("E",round(log10(Adjusted.P.value)))), hjust=-0.5, vjust = -0.9, color="black", size=5) +
  geom_text(aes(label=Row1, y = 0.2, hjust=0), vjust = -0.5, color = "White", fontface = "bold", size = 4.7) +
  geom_text(aes(label=Row2, y = 0.2, hjust=0), vjust = 1, color = "white", fontface = "bold", size = 4.7) +
...
break.across.rows(go.cov.d3.down, c(43,42,42,40,40,47,47))$Row1
go.cov.d3.down <- break.across.rows(go.cov.d3.down, c(43,42,42,40,40,47,147))
ggplot(data = go.cov.d3.down, aes(x = Term, y = -log10(Adjusted.P.value))) +
  geom_bar(stat = 'identity', fill = 'blue4') +
  geom_text(aes(label=Overlap), hjust=1.3, vjust = 1, color="Blue", size= 5) +
  geom_text(aes(label=paste("E",round(log10(Adjusted.P.value)))), hjust=1.5, vjust = -0.5, color="black", size= 5) +
  geom_text(aes(label=Row1, y = 29, hjust=0), vjust = -0.5, color = "black", fontface = "bold", size = 5) +
  geom_text(aes(label=Row2, y = 29, hjust=0), vjust = 1, color = "black", fontface = "bold", size = 5) +
...
### Viral perterbations (Up) based on Down regulaged genes

break.across.rows(go.vi.up.d3.down,split.positions = c(100,100,100,100,100,129,130))$Row1

go.vi.up.d3.down <- break.across.rows(go.vi.up.d3.down, c(100,100,100,100,100,129,130))
go.vi.up.d3.down <- go.vi.up.d3.down[1:5,]
ggplot(data = go.vi.up.d3.down, aes(x = Term, y = -log10(Adjusted.P.value))) +
  geom_bar(stat = 'identity', fill = 'blue4') +
  geom_text(aes(label=Overlap), hjust=1.3, vjust = 0.9, color="Blue", size=5) +
  geom_text(aes(label=paste("E",round(log10(Adjusted.P.value)))), hjust=1.5, vjust = -1, color="black", size=5) +
  geom_text(aes(label=Row1, y = 10, hjust=0), vjust = -0.5, color = "black", fontface = "bold", size = 5) +
  geom_text(aes(label=Row2, y = 10, hjust=0), vjust = 1.5, color = "black", fontface = "bold", size = 5) +
  scale_x_discrete(limits=rev(go.vi.up.d3.down$Term)) +
  ylim(10,0) +
  coord_flip() + 
  labs(title = 'Viral Pertubations from GEO, Up',
       subtitle = 'Day 3, Down-regulated in Infected',
       y = 'P-value, Adjusted') +
  theme(axis.text.x = element_text(size = 11),
        axis.text.y = element_blank(),
...
data.to.plot <- go.mf.d3.up; title = 'Go - Molecular Function, D3 - Up'

data.to.plot <- go.mf.d3.down; title = 'Go - Molecular Function, D3 - Down'

data.to.plot <- go.cc.d3.up; title = 'Go - Cellular Component, D3 - Up'

data.to.plot <- go.cc.d3.down; title = 'Go - Cellular Component, D3 - Down'



break.across.rows(data.to.plot, split.positions = c(19,14,100,26,26,10,10))$Row1
data.to.plot <- break.across.rows(data.to.plot, split.positions = c(19,14,100,26,26,10,10))

break.across.rows(data.to.plot, split.positions = c(13,14,100,500,100,129,130))$Row1
data.to.plot <- break.across.rows(data.to.plot, split.positions = c(13,14,100,500,100,129,130))

data.to.plot <- data.to.plot[1:5,]


plot.gene.enrichment.up(data.to.plot, title)

plot.gene.enrichment.down(data.to.plot, title)
