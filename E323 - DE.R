...
goi.rows <- which(results.cov$Gene %in% genes.of.interest)
results.goi <- results.cov[goi.rows,]
#results.goi <- results.goi[order(genes.of.interest),]
results.goi$FC.cutoff <- abs(results.goi$log2FoldChange)>0.5
results.goi$pval.cuttoff <- results.goi$padj <0.05
...
write.csv(results.goi, 'Diff. Expression of genes of interest - D 1 and 3.csv')
