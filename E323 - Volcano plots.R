...
genes.go.defense <- read.csv('C:/Users/grossar/Box/Sareen Lab Shared/Data/Vicky/E283-COVID Infection/Round 2 RNAseq/Analysis/Genes of Interest/GO_defense esponse.csv', header = TRUE)
genes.go.inflam <- read.csv('C:/Users/grossar/Box/Sareen Lab Shared/Data/Vicky/E283-COVID Infection/Round 2 RNAseq/Analysis/Genes of Interest/GO_inflam response.csv', header = TRUE)
genes.ds <- read.csv('C:/Users/grossar/Box/Sareen Lab Shared/Data/Vicky/E283-COVID Infection/Round 2 RNAseq/Analysis/Genes of Interest/highlighted-genes.csv', header = TRUE)[,1]
...
### Extract genes from gene tables
genes.go.defense <- toupper(unique(genes.go.defense$Symbol))
genes.go.inflam <- toupper(unique(genes.go.inflam$Symbol))
...
##########################################################################
### COmpose list of genes of interest

go.defense.overlap <- results.cov3[row.names(results.cov3) %in% genes.go.defense,]
go.inflam.overlap <- results.cov3[row.names(results.cov3) %in% genes.go.inflam,]

go.defense.overlap <- go.defense.overlap[order(go.defense.overlap$padj, decreasing = FALSE),]
go.inflam.overlap <- go.inflam.overlap[order(go.inflam.overlap$padj, decreasing = FALSE),]

go.defense.overlap.up <- go.defense.overlap[go.defense.overlap$log2FoldChange>0,]
go.inflam.overlap.up <- go.inflam.overlap[go.inflam.overlap$log2FoldChange>0,]
go.defense.overlap.down <- go.defense.overlap[go.defense.overlap$log2FoldChange<0,]
go.inflam.overlap.down <- go.inflam.overlap[go.inflam.overlap$log2FoldChange<0,]

summary(go.defense.overlap$padj<0.05)

setwd('C:/Users/grossar/Box/Sareen Lab Shared/Data/Vicky/E283-COVID Infection/Round 2 RNAseq/Analysis/Genes of Interest/')
write.csv(go.defense.overlap, 'DE - D3 Results - Overlap with GO Defense response.csv')
write.csv(go.inflam.overlap, 'DE - D3 Results - Overlap with GO Inflammation response.csv')

write.csv(go.defense.overlap.up, 'DE - D3 Results - Overlap with GO Defense -- UP reg in infected.csv')
write.csv(go.inflam.overlap.up, 'DE - D3 Results - Overlap with GO Inflammation -- UP reg in infected.csv')
write.csv(go.defense.overlap.down, 'DE - D3 Results - Overlap with GO Defense -- DOWN reg in infected.csv')
write.csv(go.inflam.overlap.down, 'DE - D3 Results - Overlap with GO Inflammation -- DOWN reg in infected.csv')
...
volcano.genes.of.interest.null <- volcano.data.cov[volcano.data.cov$Gene %in% genes.of.interest,]
